...
import com.luck.picture.lib.adapter.PreviewThumbnailGalleryAdapter;
...
import com.luck.picture.lib.widget.SpeedyLinearLayoutManager;
...
public class PicturePreviewActivity extends PictureBaseActivity implements
    ...
    private PreviewThumbnailGalleryAdapter thumbnailAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        recyclerView = findViewById(R.id.preview_thumbnail_recycler);
        ...
        SpeedyLinearLayoutManager mLayoutManager = new SpeedyLinearLayoutManager(this);
        mLayoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
        recyclerView.setLayoutManager(mLayoutManager);
        thumbnailAdapter = new PreviewThumbnailGalleryAdapter(this, selectImages);
        recyclerView.setAdapter(thumbnailAdapter);
        thumbnailAdapter.setOnItemClickListener(new PreviewThumbnailGalleryAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int position) {
                LocalMedia localMedia = selectImages.get(position);
                int pos = currentSelectPosition(localMedia, images);
                if (pos >= 0) {
    ...
    public void onImageChecked(int position) {
        if (images != null && images.size() > 0) {
            LocalMedia media = images.get(position);
            int pos = currentSelectPosition(media, selectImages);
            check.setSelected(pos >= 0);
            if (thumbnailAdapter != null) {
                thumbnailAdapter.changeSelectItem(recyclerView, pos);
            }
        } else {
            check.setSelected(false);
        }
     ...
     * 当前图片选中
     */
    public int currentSelectPosition(LocalMedia image, List<LocalMedia> mediaList) {
        for (int i = 0; i < mediaList.size(); i++) {
            if (mediaList.get(i).getPath().equals(image.getPath())) {
                return i;
            }
        }
        return -1;
...
//                tv_img_num.setText(String.valueOf(selectImages.size()));
    ...
    private void updateSelector(boolean isRefresh) {
        ...
        if (thumbnailAdapter != null) {
            int pos = currentSelectPosition(images.get(index), selectImages);
            thumbnailAdapter.changeSelectItem(recyclerView,pos);
            thumbnailAdapter.bindFolderData(selectImages);
        }
    }
